...
MydbcDatabase::MydbcDatabase(void)
...
BOOL MydbcDatabase::Initialize( MydbcDatabaseDesc & desc )
{
	...
	case DBCInterfaceType_MYSQL:
		{
			m_ppDBCInstance = new MydbcInterface*[m_wQueryProcessorNum];
			for( WORD i = 0 ; i < m_wQueryProcessorNum ; ++i )
				m_ppDBCInstance[i] = new MydbcMysql;
		}
		break;
	...
	m_pQueryCS = new UtilityMutex;
	m_pQueryResultCS = new UtilityMutex;

	m_pQueryList = new UtilityLinkedList<MydbcQueryResult *>( desc.dwQueryPoolSize );
	m_pQueryResultPushList = new UtilityLinkedList<MydbcQueryResult *>( desc.dwQueryPoolSize );
	m_pQueryResultPopList = new UtilityLinkedList<MydbcQueryResult *>( desc.dwQueryPoolSize );
...
BOOL MydbcDatabase::DBQuery( MydbcQueryResult * pQuery )
{
	ASSERT( pQuery );
...
VOID MydbcDatabase::Update()
{
	...
	*/
	...
	while( bInfiniteLoop )
	{
		MydbcQueryResult * pQResult = m_pQueryResultPopList->DeleteHead();
