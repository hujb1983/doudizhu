...
BOOL MydbcMysql::SQLError( const char* pszErr, MydbcQueryResult * pQuery )
{
	char Buffer[512];
	if ( pQuery )
	{
...
BOOL MydbcMysql::Init( char * pszIPAddress, char * pszDatabaseName, char * pszUserName, char * pszUserPassword, fnCallBackMessage fnErrorMessage )
{
...
VOID MydbcMysql::Disconnect()
{
	mysql_close( m_pMySql );
}

BOOL MydbcMysql::Connect()
{
	ASSERT( m_pMySql );
...
// 如果断开则重连
void MydbcMysql::TryConnect(void)
{
...
BOOL MydbcMysql::ExecuteLargeData( MydbcQueryResult * pQuery )
{
	// 如果断开则重连
	TryConnect();
...
BOOL MydbcMysql::Execute( MydbcQueryResult * pQuery )
{
	// 如果断开则重连
	TryConnect();
...
BOOL MydbcMysql::SetField( const char* pSrc, SHORT sType, ULONG* pDst, ULONG nDstLen, ULONG nSrcLen )
{
	//add by df
	if(NULL == pSrc)
		return false;
