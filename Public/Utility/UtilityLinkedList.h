#ifndef _UtilityLinkedList_H_
#define _UtilityLinkedList_H_
#include <UtilityMemoryFactory.h>

typedef void* POS;

template <class T, class numType = DWORD >
class UtilityLinkedList
{
public:
	UtilityLinkedList(DWORD dwBlockSize = 10) : m_pHead(0), m_pTail(0), m_nCount(0)
	{
		m_pNodePool = new UtilityMemoryFactory<NODE>;
		m_pNodePool->Initialize(dwBlockSize, dwBlockSize/2+1);
	}
	virtual ~UtilityLinkedList()
	{
		DeleteAll();
		m_pNodePool->Release();
		delete m_pNodePool;
...
protected:
    ...
    NODE *	m_pTail;
    numType	m_nCount;
	UtilityMemoryFactory<NODE>	* m_pNodePool;
