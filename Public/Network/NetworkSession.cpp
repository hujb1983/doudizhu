...
using namespace NetworkSocketOption;

NetworkSession::NetworkSession(DWORD dwSendBufferSize, DWORD dwRecvBufferSize, DWORD dwMaxPacketSize, DWORD dwTimeOut)
{
	m_pSendBuffer = new NetworkSendBuffer;
	m_pSendBuffer->Create(dwSendBufferSize, dwMaxPacketSize);
	m_pSendBuffer->Clear();

	m_pRecvBuffer = new NetworkRecvBuffer;
...
BOOL NetworkSession::Send( BYTE * pMsg, WORD wSize )
{
...
BOOL NetworkSession::SendEx( DWORD dwNumberOfMessages, BYTE ** ppMsg, WORD *pwSize)
{
	PACKET_HEADER header;
	header.size = 0;
	DWORD i;
...
BOOL NetworkSession::ProcessRecvdPacket( DWORD dwMaxPacketSize)
{
	BYTE 			* pPacket;
...
BOOL NetworkSession::PreSend(NetworkSyncHandler * pSync)
{
...
BOOL NetworkSession::DoSend(NetworkSyncHandler * pSync)
{
	UtilityGuard gd(m_lockSend);
...
BOOL NetworkSession::CreateSocket()
{
	int nRet		= 0;
	int nZero		= 0;
	...
	NetworkSocketOption::Nonblocking( newSocket );
	NetworkSocketOption::DisableBuffering( newSocket );
	return newSocket;
}

void NetworkSession::BindNetworkObject( NetworkObject *pNetworkObject )
{
	m_pNetworkObject = pNetworkObject;
	pNetworkObject->SetSession( this );
}

void NetworkSession::UnbindNetworkObject()
{
	if( m_pNetworkObject == NULL ) {
		return;
	}
...
void NetworkSession::OnConnect( BOOL bSuccess )
{
	this->Init();
...
void NetworkSession::Disconnect( BOOL bGracefulDisconnect )
{
	printf( "[NetworkSession::Disconnect = %d \n]", bGracefulDisconnect ) ;
...
{
	NetworkSocketOption::CloseSocket(m_socket);
	m_socket = INVALID_SOCKET;
}

DWORD NetworkSession::GetTickCount()
{
...
void NetworkSession::OnLogString(char * pszLog, ...)
{
	if ( !m_pNetworkObject )
		return;
