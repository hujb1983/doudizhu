...
{
	FUNC_Client * pFuncInfo = (FUNC_Client *)m_pFuncMap_Client->Find( dwProtocol );
	if (pFuncInfo) {
...
{
	assert(NULL != pMsg);
	FUNC_Login * pFuncInfo = (FUNC_Login *)m_pFuncMap_Login->Find( dwProtocol );
	if (pFuncInfo) {
		pFuncInfo->m_fnHandler( pServerSession, pMsg, wSize );
	}
}
...
{
	assert(NULL != pMsg);
	FUNC_Agent * pFuncInfo = (FUNC_Agent *)m_pFuncMap_Agent->Find( dwProtocol );
	if (pFuncInfo) {
		pFuncInfo->m_fnHandler( pServerSession, pMsg, wSize );
	}
}
...
{
	assert(NULL != pMsg);
	FUNC_Lobby * pFuncInfo = (FUNC_Lobby *)m_pFuncMap_Lobby->Find( dwProtocol );
	if (pFuncInfo) {
		pFuncInfo->m_fnHandler( pServerSession, pMsg, wSize );
	}
}
...
{	
	assert(NULL != pMsg);
	FUNC_Games * pFuncInfo = (FUNC_Games *)m_pFuncMap_Games->Find( dwProtocol );
	if (pFuncInfo) {
		pFuncInfo->m_fnHandler( pServerSession, pMsg, wSize );
	}
}
...
{
	assert(NULL != pMsg);
	FUNC_DB * pFuncInfo = (FUNC_DB *)m_pFuncMap_DB->Find( dwProtocol );
	if (pFuncInfo) {
		pFuncInfo->m_fnHandler( pServerSession, pMsg, wSize );
	}
}
